...
For traditional non-libadwaita GTK 4 apps, use the [`gtk4`](./gtk4.md) plugin instead — it writes a different file into the same directory.
...
If you also use the [`gtk4`](./gtk4.md) plugin, you'll have two `@import` lines (order doesn't matter — they define disjoint sets of variables):
...
If you're on a recent version and apps still look unchanged, the app may be a non-libadwaita GTK 4 app — try the [`gtk4`](./gtk4.md) plugin instead.
...
- [gtk4](./gtk4.md) — companion plugin for traditional GTK 4 apps; writes a different file in the same `~/.config/gtk-4.0/` directory.
