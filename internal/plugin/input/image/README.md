...
plugin:
  ...
  source: builtin
  source_type: image
  description: Extract colours from images (files, directories, URLs) with optional ambient lighting
  requires: []
  optional: []
...
- [`file`](../file/) — load a saved tinct palette instead of re-extracting
- [`markdown`](../markdown/) — restore a theme exported by the `markdown` output plugin (including the embedded wallpaper)
- [`google-genai`](../googlegenai/), [`openrouter`](../openrouter/) — generate the source image with an AI model instead of supplying one
