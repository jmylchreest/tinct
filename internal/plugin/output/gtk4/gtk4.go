...
// PostExecute provides instructions for configuring GTK4 and attempts live reload via D-Bus.
func (p *Plugin) PostExecute(ctx context.Context, execCtx output.ExecutionContext, _ []string) error {
	...
	if err != nil && p.verbose {
		...
		fmt.Fprintf(os.Stderr, "   GTK4 theme reload triggered via D-Bus (running apps should update)\n")
	}
