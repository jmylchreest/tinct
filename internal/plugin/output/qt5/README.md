...
Probably not. Plasma supplies its own `QPlatformTheme` (`plasma`) that overrides `QT_QPA_PLATFORMTHEME=qt5ct`. Use the [`kde-plasma`](../kde-plasma/README.md) plugin instead, which writes a proper KDE colour scheme.

## Related plugins

- [qt6](../qt6/README.md) — sibling plugin for Qt 6 apps (writes to `~/.config/qt6ct/colors/`).
