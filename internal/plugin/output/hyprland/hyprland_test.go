...
func TestHyprlandPlugin_ContentValidation(t *testing.T) {
	...
	// Check that hex values are actually present (not just "rgb(#)")
	if strings.Contains(content, "rgb(#)") {
		t.Error("Generated content contains invalid rgb(#) - hex values not being generated")
	}

	// Verify RGB format has 6 character hex values
	if !strings.Contains(content, "rgb(") {
		t.Error("Generated content missing rgb() color format")
	}
}
