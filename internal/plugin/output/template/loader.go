...
func (l *Loader) Load(filename string) (content []byte, fromCustom bool, err error) {
	...
	if l.targetVersion != "" {
		...
		// Log why we're falling back to default (only if verbose and no versioned template found)
		if l.verbose && l.logger != nil && vErr != nil {
			l.logger.Printf("   Versioned template not available for %s: %v", filename, vErr)
		}
		// Note: We don't log when versionedContent is nil because that just means
		// this specific file doesn't have a versioned variant, which is normal
	}
...
func (l *Loader) loadVersionedTemplate(filename string) ([]byte, string, error) {
	// Find available version directories
...
func (l *Loader) findVersionDirectories() ([]string, error) {
	...
	if err != nil {
		// templates directory doesn't exist - that's fine
		return nil, nil
	}

	for _, entry := range entries {
		if entry.IsDir() {
			// Check if the directory name is a valid version
	...
	// Sort versions for consistent behavior
