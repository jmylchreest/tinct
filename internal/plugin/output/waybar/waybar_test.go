...
func TestWaybarPlugin(t *testing.T) {
	...
	config := plugintesting.TestConfig{
		ExpectedName:       "waybar",
		ExpectedFiles:      []string{"tinct.css", "../tinct-style.css"},
...
func TestWaybarPlugin_ContentValidation(t *testing.T) {
	...
	// Check stub CSS file.
	stubCSS := string(files["../tinct-style.css"])
	requiredStubStrings := []string{
		"Waybar",
		"@import",
		"themes/",
		"tinct.css",
	...
	for _, required := range requiredStubStrings {
		if !strings.Contains(stubCSS, required) {
			t.Errorf("Stub CSS missing required string: %s", required)
		}
	}

	// Check colours CSS file.
	coloursCSS := string(files["tinct.css"])
