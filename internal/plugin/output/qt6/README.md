...
For KDE Plasma users, prefer the [`kde-plasma`](../kde-plasma/README.md) plugin — Plasma's colour scheme system is richer and applies system-wide.
...
Probably not. Plasma supplies its own `QPlatformTheme` (`plasma`) that overrides `QT_QPA_PLATFORMTHEME=qt6ct`. Use the [`kde-plasma`](../kde-plasma/README.md) plugin instead, which writes a proper KDE colour scheme that applies to both Qt 5 and Qt 6 apps.

## Related plugins

- [qt5](../qt5/README.md) — sibling plugin for legacy Qt 5 apps (writes to `~/.config/qt5ct/colors/`).
- [kde-plasma](../kde-plasma/README.md) — preferred for KDE Plasma users; applies to both Qt 5 and Qt 6 apps system-wide.
- [gtk3](../gtk3/README.md), [gtk4](../gtk4/README.md) — pair with these so Qt and GTK apps look consistent.
